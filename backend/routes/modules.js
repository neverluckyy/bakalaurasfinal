  ...
  return new Promise((resolve, reject) => {
    ...
    db.get(
      ...
      (err, lcResult) => {
        ...
        const learningContentCompleted = hasLearningContent ? (lcResult.completed === lcResult.total) : false;
        ...
        db.get(
          ...
          (err, quizResult) => {
            ...
            // Section completion semantics for unlocking:
            // - If the section has a quiz: passing the quiz is sufficient to complete the section.
            // - If the section has no quiz but has learning content: completing learning content is sufficient.
            // This matches user expectations: "pass previous section -> next unlocks".
            const sectionCompleted = hasQuiz
              ? quizPassed
              : (hasLearningContent ? learningContentCompleted : false);
...
router.get('/:moduleId/sections', authenticateToken, async (req, res) => {
  ...
  const query = `
    SELECT DISTINCT
      ...
      (
        SELECT COUNT(DISTINCT up2.question_id)
        FROM questions q3
