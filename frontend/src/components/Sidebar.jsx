...
function Sidebar({ isCollapsed, onToggle }) {
  ...
  const navItems = [
    ...
    { path: '/support', icon: HelpCircle, label: 'Support', category: 'ACCOUNT' },
  ];
      ...
      <div className={`sidebar ${isCollapsed ? 'collapsed' : ''} ${isMobileOpen ? 'mobile-open' : ''}`}>
        <div className="sidebar-content">
          ...
          <div className="nav-section">
            {(!isCollapsed || isMobile) && <h3 className="nav-category">LEGAL</h3>}
            <nav className="nav-list">
              <Link
                to="/terms"
                className={`nav-item ${location.pathname === '/terms' ? 'active' : ''}`}
                title={isCollapsed && !isMobile ? 'Terms of Use' : ''}
                onClick={() => isMobile && setIsMobileOpen(false)}
              >
                <FileText className="nav-icon" />
                {(!isCollapsed || isMobile) && <span className="nav-label">Terms of Use</span>}
              </Link>
              <Link
                to="/privacy"
                className={`nav-item ${location.pathname === '/privacy' ? 'active' : ''}`}
                title={isCollapsed && !isMobile ? 'Privacy Policy' : ''}
                onClick={() => isMobile && setIsMobileOpen(false)}
              >
                <ShieldCheck className="nav-icon" />
                {(!isCollapsed || isMobile) && <span className="nav-label">Privacy Policy</span>}
              </Link>
            </nav>
          </div>
