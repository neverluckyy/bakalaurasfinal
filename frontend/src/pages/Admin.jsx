...
function Admin() {
  ...
  const fetchQuestions = async (sectionId) => {
    ...
    try {
      ...
      const errorMsg = err.response?.data?.error || err.message || 'Failed to load questions';
      setError(`Failed to load questions: ${errorMsg}`);
  ...
  const handleDeleteSection = async (id) => {
    ...
    setMessage('');
    try {
      const response = await axios.delete(`/api/admin/sections/${id}`, { withCredentials: true });
      setMessage(response.data?.message || 'Section deleted successfully');
