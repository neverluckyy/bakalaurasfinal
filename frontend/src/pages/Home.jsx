...
function Home() {
  ...
  const fetchModules = useCallback(async () => {
    try {
      ...
      if (!hasProgress) {
        try {
          const prefix = `u_${user?.id || 'anon'}_`;
          // Check all localStorage keys for reading positions or quiz drafts
          for (let i = 0; i < localStorage.length; i++) {
            const key = localStorage.key(i);
            if (key && key.startsWith(prefix) && (key.includes('_reading_position_') || key.includes('_quiz_draft_'))) {
              try {
                const data = JSON.parse(localStorage.getItem(key));
                // Check if it's actual progress (not just step 0 or empty)
                if (key.includes('_reading_position_')) {
                  if (data.stepIndex > 0) {
                    hasProgress = true;
                    break;
                  }
                } else if (key.includes('_quiz_draft_')) {
      ...
      try {
        ...
        for (let i = 0; i < localStorage.length; i++) {
          ...
          if (key && key.startsWith(prefix) && (key.includes('_reading_position_') || key.includes('_quiz_draft_'))) {
            try {
              const data = JSON.parse(localStorage.getItem(key));
              if (key.includes('_reading_position_') && data.stepIndex > 0) {
                hasProgress = true;
                break;
              } else if (key.includes('_quiz_draft_') && data.draftAnswers && Object.keys(data.draftAnswers).length > 0) {
                hasProgress = true;
                break;
              }
            } catch (parseErr) {
  ...
  }, [user, checkAuth, fetchModules]); // Include dependencies to satisfy ESLint
