...
const SectionQuiz = () => {
  ...
  const getQuizDraftStorageKey = useCallback(() => {
    return `u_${user?.id || 'anon'}_quiz_draft_${sectionId}`;
  }, [user?.id, sectionId]);
  ...
  // Helper function to apply saved question order
  const applyQuestionOrder = useCallback((questionsData, savedOrder) => {
    if (!savedOrder || !Array.isArray(savedOrder) || savedOrder.length !== questionsData.length) {
      return null; // Invalid order, need to randomize
    }
    
    // Verify saved order is valid (contains all indices 0 to length-1)
    const sortedOrder = [...savedOrder].sort((a, b) => a - b);
    const isValid = sortedOrder.every((val, idx) => val === idx);
    if (!isValid) {
    ...
    // Randomize answer order for each question (this can still be random)
    const answerOrder = {};
    savedOrder.forEach((originalIndex) => {
      const question = questionsData[originalIndex];
      if (question.options && Array.isArray(question.options)) {
        const optionIndices = question.options.map((_, idx) => idx);
        const randomizedOptionIndices = shuffleArray(optionIndices);
    ...
    // Return questions with saved question order but randomized answer options
    return savedOrder.map((originalIndex) => {
      const question = questionsData[originalIndex];
      if (answerOrder[originalIndex] && question.options) {
        const randomizedOptions = answerOrder[originalIndex].map(
          (optIdx) => question.options[optIdx]
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        ...
        // Always check localStorage for questionOrder since backend doesn't store it
        let savedDraft = null;
        let localStorageDraft = null;
        
        // First, try to get from localStorage (has questionOrder)
        try {
          const storageKey = getQuizDraftStorageKey();
          const savedData = localStorage.getItem(storageKey);
          if (savedData) {
            const parsed = JSON.parse(savedData);
            localStorageDraft = {
              currentQuestionIndex: parsed.currentQuestionIndex !== undefined ? parsed.currentQuestionIndex : null,
              draftAnswers: parsed.draftAnswers || {},
              questionOrder: parsed.questionOrder || null
            };
          }
        } catch (storageErr) {
        ...
        // Try to get from API (may have more recent data)
        try {
          const draftResponse = await axios.get(`/api/sections/${sectionId}/quiz/draft`);
          const apiDraft = draftResponse.data;
          
          // Merge API data with localStorage questionOrder
          savedDraft = {
            currentQuestionIndex: apiDraft.currentQuestionIndex !== undefined ? apiDraft.currentQuestionIndex : (localStorageDraft?.currentQuestionIndex || null),
            draftAnswers: apiDraft.draftAnswers || localStorageDraft?.draftAnswers || {},
            questionOrder: localStorageDraft?.questionOrder || null // Always use localStorage for questionOrder
          };
        } catch (err) {
          console.error('Error fetching draft from server:', err);
          // Use localStorage draft if available
          savedDraft = localStorageDraft;
        }
        ...
        // If we have a saved draft with question order, use that order instead of randomizing
        if (savedDraft && savedDraft.questionOrder && Array.isArray(savedDraft.questionOrder)) {
          const questionsWithOrder = applyQuestionOrder(questionsResponse.data, savedDraft.questionOrder);
          if (questionsWithOrder) {
            // Successfully restored saved order
            randomizedQuestions = questionsWithOrder;
            
            // Restore answers using the saved order
            if (savedDraft.draftAnswers && Object.keys(savedDraft.draftAnswers).length > 0) {
              // Map saved answers from original indices to current randomized indices
              randomizedQuestions.forEach((q, randomizedIndex) => {
                ...
                if (savedDraft.draftAnswers[originalIndex] !== undefined) {
                  restoredAnswers[randomizedIndex] = savedDraft.draftAnswers[originalIndex];
                }
              });
              
              // Recalculate score from saved answers
              randomizedQuestions.forEach((q, randomizedIndex) => {
                if (restoredAnswers[randomizedIndex] && restoredAnswers[randomizedIndex] === q.correct_answer) {
                  restoredScore += 1;
                }
              });
            }
            ...
            if (savedDraft.currentQuestionIndex !== null && savedDraft.currentQuestionIndex !== undefined) {
              ...
              if (savedOriginalIndex >= 0 && savedOriginalIndex < questionsResponse.data.length) {
                ...
                if (randomizedIndex >= 0 && randomizedIndex < randomizedQuestions.length) {
                  restoredCurrentQuestion = randomizedIndex;
                }
              }
            }
          } else {
            // Saved order is invalid, randomize fresh
            randomizedQuestions = randomizeQuiz(questionsResponse.data);
          }
        } else {
          // No saved draft, randomize fresh
          randomizedQuestions = randomizeQuiz(questionsResponse.data);
        }
    ...
    fetchQuestions();
  }, [sectionId, randomizeQuiz, getQuizDraftStorageKey, applyQuestionOrder]);
  ...
  const handleNext = async () => {
    if (currentQuestion < questions.length - 1) {
      ...
      try {
        ...
        localStorage.removeItem(getQuizDraftStorageKey());
  ...
  useEffect(() => {
    const saveToStorage = () => {
      if (questions.length > 0 && !quizCompleted) {
        ...
        localStorage.setItem(getQuizDraftStorageKey(), JSON.stringify({
          currentQuestionIndex: currentOriginalIndex,
          draftAnswers: answersByOriginalIndex,
          questionOrder: questionOrder,
          timestamp: Date.now()
  ...
  }, [currentQuestion, answers, sectionId, questions.length, quizCompleted, questionOrder, questions, getQuizDraftStorageKey]);
